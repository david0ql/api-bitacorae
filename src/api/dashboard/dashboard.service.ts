...
//*********** */
		const cohorts = await this.dataSource.query(`SELECT id, name, start_date, end_date FROM cohort c ORDER BY c.order`)
...
//*********** */
		const globalDataRaw = await this.dataSource.query(`
			WITH RECURSIVE month_series AS (
				SELECT DATE_FORMAT(
					(SELECT LEAST(
...
//*********** */
		const consultorTypes = await this.dataSource.query(`
			SELECT
				ct.id,
				ct.name,
		...
		`)
//*********** */
		const economicActivity = (await this.dataSource.query(`
			SELECT
				ea.name AS name,
				COUNT(*) AS value
			FROM
		...
		const chartData6 = {
			...
			total: economicActivity.reduce((acc, curr) => acc + curr.value, 0)
		}
//*********** */
		const strengtheningArea = (await this.dataSource.query(`
			SELECT
				sa.name AS name,
				COUNT(*) AS value
			FROM
		...
		const chartData7 = {
			strengthening_area: strengtheningArea,
			total: strengtheningArea.reduce((acc, curr) => acc + curr.value, 0)
		}
