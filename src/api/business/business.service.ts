...
import {
  createPublicBulkToken,
  getBusinessNameFromPublicBulkToken,
  getPublicBulkTokenInfo,
} from 'src/utils/public-bulk-token.util';
...
export class BusinessService {
  ...
  private getBulkColumns(): BulkColumn[] {
    return [
      ...
      {
        key: 'contact_strengthening_area_ids',
        required: true,
        type: 'numberArray',
        catalog: 'strengtheningAreas',
      },
      {
        key: 'contact_education_level_id',
        required: true,
        type: 'number',
        catalog: 'educationLevels',
      },
      ...
      {
        key: 'business_economic_activity_ids',
        required: true,
        type: 'numberArray',
        catalog: 'economicActivities',
      },
      {
        key: 'business_size_id',
        required: true,
        type: 'number',
        catalog: 'businessSizes',
      },
      {
        key: 'business_service_id',
        required: true,
        type: 'number',
        catalog: 'services',
      },
      ...
      {
        key: 'business_number_of_people_leading',
        required: true,
        type: 'number',
      },
      {
        key: 'business_product_status_id',
        required: true,
        type: 'number',
        catalog: 'productStatuses',
      },
      {
        key: 'business_market_scope_id',
        required: true,
        type: 'number',
        catalog: 'marketScopes',
      },
      { key: 'business_business_plan', required: true, type: 'string' },
      { key: 'business_business_segmentation', required: true, type: 'string' },
      {
        key: 'business_strengthening_area_ids',
        required: true,
        type: 'numberArray',
        catalog: 'strengtheningAreas',
      },
  ...
  private async loadBulkCatalogs(dataSource: any) {
    const [
      ...
      dataSource
        ...
        .find({ select: { id: true, name: true }, order: { id: 'ASC' } }),
      dataSource
      ...
      dataSource
        ...
        .find({ select: { id: true, name: true }, order: { id: 'ASC' } }),
      dataSource
        .getRepository(EconomicActivity)
        .find({ select: { id: true, name: true }, order: { id: 'ASC' } }),
      dataSource
        .getRepository(BusinessSize)
        .find({ select: { id: true, name: true }, order: { id: 'ASC' } }),
      dataSource
      ...
      dataSource
        ...
        .find({ select: { id: true, name: true }, order: { id: 'ASC' } }),
      dataSource
        .getRepository(ProductStatus)
        .find({ select: { id: true, name: true }, order: { id: 'ASC' } }),
      dataSource
        .getRepository(MarketScope)
        .find({ select: { id: true, name: true }, order: { id: 'ASC' } }),
      dataSource
        .getRepository(Cohort)
        .find({ select: { id: true, name: true }, order: { id: 'ASC' } }),
      dataSource
        .getRepository(StrengtheningArea)
        .find({ select: { id: true, name: true }, order: { id: 'ASC' } }),
    ]);
  ...
  async getBulkCatalogs(businessName: string) {
    if (!businessName) {
      throw new BadRequestException(
        'Se requiere especificar una empresa para consultar catalogos',
      );
    }
  ...
  getPublicBulkLinkToken(businessName: string) {
    if (!businessName) {
      throw new BadRequestException(
        'Se requiere especificar una empresa para generar el enlace público',
      );
    }
  ...
  async create(
    ...
    if (!businessName) {
      throw new BadRequestException(
        'Se requiere especificar una empresa para crear el negocio',
      );
    }
    ...
    const primaryFilePath =
      uploadedFiles.length > 0
        ? this.fileUploadService.getFullPath(
            'business',
    ...
    try {
      ...
      const economicActivityRepository =
        businessDataSource.getRepository(EconomicActivity);
      const strengtheningAreaRepository =
        businessDataSource.getRepository(StrengtheningArea);
      ...
      if (existingUser) {
        uploadedFiles.forEach((uploadedFile) => {
          const filePath = this.fileUploadService.getFullPath(
            'business',
      ...
      const strengtheningAreaEntities =
        await strengtheningAreaRepository.findBy({
      ...
      if (!service) {
        throw new BadRequestException(
          `No se encontró un servicio con el ID ${serviceId}`,
        );
      }
  ...
  async getBulkTemplate(businessName: string): Promise<Buffer> {
    ...
    const exampleRow = {
      ...
      business_economic_activity_ids: catalogs.economicActivities
        .slice(0, 2)
        .map((item) => item.id)
        .join(','),
  ...
  async bulkUpload(
    ...
    if (!businessName) {
      throw new BadRequestException(
        'Se requiere especificar una empresa para el cargue masivo',
      );
    }
    ...
    try {
      ...
      const sheet =
        workbook.getWorksheet('Plantilla') || workbook.worksheets[0];
      if (!sheet) {
        throw new BadRequestException(
          'No se encontro la hoja Plantilla en el archivo',
        );
      }
      ...
      const catalogSets: Record<CatalogKey, Set<number>> = {
        ...
        educationLevels: new Set(
          catalogs.educationLevels.map((item) => item.id),
        ),
        economicActivities: new Set(
          catalogs.economicActivities.map((item) => item.id),
        ),
        ...
        productStatuses: new Set(
          catalogs.productStatuses.map((item) => item.id),
        ),
        marketScopes: new Set(catalogs.marketScopes.map((item) => item.id)),
        cohorts: new Set(catalogs.cohorts.map((item) => item.id)),
        strengtheningAreas: new Set(
          catalogs.strengtheningAreas.map((item) => item.id),
        ),
      };

      const existingEmails = new Set(
        (
          await businessDataSource
            .getRepository(User)
      ...
      for (let rowIndex = 2; rowIndex <= sheet.rowCount; rowIndex += 1) {
        ...
        for (const column of columns) {
          ...
          if (column.catalog && parsed[column.key]) {
            const idsToCheck = Array.isArray(parsed[column.key])
              ? parsed[column.key]
              : [parsed[column.key]];
            const invalidIds = idsToCheck.filter(
              (id) =>
                !catalogSets[column.catalog as CatalogKey].has(Number(id)),
            );
            if (invalidIds.length > 0) {
              rowErrors.push(
                `${column.key} tiene IDs invalidos: ${invalidIds.join(', ')}`,
              );
            }
          }
        }

        const businessEmail = String(parsed.business_email || '')
          .trim()
          .toLowerCase();
        if (businessEmail) {
      ...
      const createdEvidenceAttachments: {
        businessId: number;
        evidenceUrl: string;
      }[] = [];
      let created = 0;
      ...
      for (const row of rowsToCreate) {
        ...
        try {
          const savedBusiness = await businessDataSource.transaction(
            async (manager) => {
              const userRepository = manager.getRepository(User);
              const businessRepository = manager.getRepository(Business);
              const contactRepository =
                manager.getRepository(ContactInformation);
              const economicActivityRepository =
                manager.getRepository(EconomicActivity);
              const strengtheningAreaRepository =
                manager.getRepository(StrengtheningArea);

              const salt = bcrypt.genSaltSync(10);
              const hash = bcrypt.hashSync(
                String(data.business_password),
                salt,
              );
              ...
              const economicActivityEntities =
                await economicActivityRepository.findBy({
                  id: In(data.business_economic_activity_ids),
                });
              const businessStrengtheningEntities =
                await strengtheningAreaRepository.findBy({
              ...
              const contactStrengtheningEntities =
                await strengtheningAreaRepository.findBy({
      ...
      if (createdEvidenceAttachments.length > 0) {
        for (const attachment of createdEvidenceAttachments) {
          try {
            ...
            console.error(
              'Error creating evidence attachment from bulk upload:',
              e,
            );
          }
        }
    ...
    try {
      const sql = `
				SELECT
					...
					srv.level_id AS serviceLevelId,
					level.name AS serviceLevelName,
				...
				FROM
					...
					LEFT JOIN strengthening_level level ON level.id = srv.level_id
  ...
  async findOne(id: number, businessName: string) {
    ...
    try {
      ...
      const evidenceUrl =
        evidenceFiles?.[0]?.fileUrl ||
        (business.evidence ? this.constructFileUrl(business.evidence) : null);

      const serviceWithLevelName = business.service
        ? {
            ...business.service,
            levelName: business.service.level?.name || '',
          }
        : null;

      return {
        ...business,
        service: serviceWithLevelName,
  ...
  async update(
    ...
    if (!businessName) {
      throw new BadRequestException(
        'Se requiere especificar una empresa para actualizar el negocio',
      );
    }
    await this.requestAttachmentService.ensureHomologated(businessName);
    ...
    try {
      ...
      if (!existingBusiness) {
        throw new BadRequestException(
          `No se encontró un negocio con el ID ${id}`,
        );
      }
      ...
      if (updateBusinessDto.serviceId !== undefined) {
        const service = await serviceRepository.findOne({
          where: { id: updateBusinessDto.serviceId },
        });
        if (!service) {
          throw new BadRequestException(
            `No se encontró un servicio con el ID ${updateBusinessDto.serviceId}`,
          );
        }
        existingBusiness.serviceId = service.id;
      } else if (!existingBusiness.serviceId) {
        throw new BadRequestException(
          'El servicio es obligatorio para actualizar la empresa',
        );
      }
      ...
      const { economicActivities, strengtheningAreas, ...updateData } =
        updateBusinessDto;
      ...
      const evidenceUrl =
        evidenceFiles?.[0]?.fileUrl ||
        (updatedBusiness.evidence
          ? this.constructFileUrl(updatedBusiness.evidence)
          : null);
